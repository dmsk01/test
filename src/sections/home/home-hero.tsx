...
export function HomeHero({ sx, ...other }: BoxProps) {
  ...
  const mdUp = useMediaQuery((th) => th.breakpoints.up(mdKey));
    ...
    <m.div {...motionProps}>
      <Box
        ...
        sx={{
          ...
          [theme.breakpoints.up(lgKey)]: { fontSize: 60, lineHeight: '80px' },
        }}
      >
        {t('pages.home.title').split(' ').slice(0, -2).join(' ')}
        <Box
          component="span"
          sx={{
            typography: 'span',
        ...
        >
          {t('pages.home.title').split(' ').slice(-2).join(' ')}
        </Box>
      </Box>
    </m.div>
  );
