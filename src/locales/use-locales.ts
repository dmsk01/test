...
export function useTranslate(ns?: string) {
  ...
  const currentLang = allLangs.find((lang) => lang.value === i18n.resolvedLanguage) ?? fallback;
  ...
  const onChangeLang = useCallback(
    async (newLang: LanguageValue) => {
      try {
        ...
        const foundLang = allLangs.find((lang) => lang.value === newLang);
        if (foundLang) {
          dayjs.locale(foundLang.adapterLocale);
        }
        ...
        document.cookie = `${cookieName}=${newLang}; path=/`;
      } catch (error) {
        console.error(error);
      }
    },
