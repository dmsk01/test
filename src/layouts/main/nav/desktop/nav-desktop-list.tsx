...
export function NavList({ data, sx, ...other }: NavListProps) {
  ...
  const { currentLang: locale } = useTranslate();
  const normalizedPathname = pathname.replace(`/${locale.value}`, '') || '/';
  const isActive = isActiveLink(normalizedPathname, data.path, !!data.children);
